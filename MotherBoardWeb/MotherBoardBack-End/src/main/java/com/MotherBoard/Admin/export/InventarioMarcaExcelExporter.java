...
public class InventarioMarcaExcelExporter extends AbstractExporter{
	
	private XSSFWorkbook workbook;
	private XSSFSheet sheet;
	
	public InventarioMarcaExcelExporter() {
		workbook = new XSSFWorkbook();
	}
	
	private void writeHeaderLine() {
	    sheet = workbook.createSheet("InventarioMarcas");
	    ...
	    createCell(row, 5, "ID da Marca", cellStyle);
	    createCell(row, 6, "Ação", cellStyle);
	    createCell(row, 7, "Data de Modificação", cellStyle);
	}

	private void writeDataLines(List<InventarioMarca> listInventarioMarcas) {
	    int rowIndex = 1;
	    XSSFCellStyle cellStyle = workbook.createCellStyle();
	    XSSFFont font = workbook.createFont();
	    ...
	    for (InventarioMarca inventarioMarca : listInventarioMarcas) {
	        ...
	        createCell(row, columnIndex++, inventarioMarca.getId(), cellStyle);
	        ...
	        createCell(row, columnIndex++, inventarioMarca.getRoleUsuario(), cellStyle);
	        
	        Marca marca = inventarioMarca.getMarca(); 
	        createCell(row, columnIndex++, marca != null ? marca.getNome() : "", cellStyle);
	        createCell(row, columnIndex++, marca != null ? marca.getId() : "", cellStyle);
	        
	        createCell(row, columnIndex++, inventarioMarca.getAcao(), cellStyle);
	        createCell(row, columnIndex++, inventarioMarca.getDataModificacao(), cellStyle);
	    }
	}
	...
	public void export(List<InventarioMarca> listInventarioMarcas, HttpServletResponse response) throws IOException {
	    if (listInventarioMarcas == null) {
	    	listInventarioMarcas = new ArrayList<>();
	    ...
	    writeHeaderLine();
	    writeDataLines(listInventarioMarcas);
